...
> build this project **under Claude Code**. It tells you what to build, in what order, why each piece
> exists, and — critically — **what to take as-is, what to adapt, and what to leave behind.**
>
...
>
> **Harness note.** This repo is driven by **Claude Code**. Project skills live in `.claude/skills/`
> (auto-discovered and invoked via the Skill tool, i.e. `/skill-name`); the always-loaded instruction
> file is `CLAUDE.md`; isolation uses Claude Code's worktree tooling. The workflow itself is
> harness-agnostic — only these paths/mechanisms change between agent runtimes.
...
```
Layer 3 — INSTRUCTION FILE (CLAUDE.md, always loaded)
   Hard ceilings ("never do X"), product invariants, and ROUTES into the skills.
   Says WHAT MUST HOLD and WHERE TO LOOK — not how.
        │ routes to
        ▼
Layer 2 — PROJECT SKILLS (.claude/skills/, committed to the repo)
   Project-shaped procedures every agent shares: how we ship a change, how we build & test.
   Auto-discovered by Claude Code; invoked via the Skill tool (/skill-name). Compose Layer 1.
        │ composes / references by name
        ▼
Layer 1 — GENERIC WORKFLOW SKILLS ("superpowers", Claude user-level ~/.claude/skills/)
...
**TAKE (ported here):** change-risk tiers; a tiny always-loaded `CLAUDE.md` with checkable ceilings;
...
**Layer 1 — generic skills:** the "superpowers" bundle, installed once at the **Claude user level**
(`~/.claude/skills/`) and surfaced through the Skill tool (`/skill-name`): `using-superpowers`,
...
**Layer 2 — project skills (created in this repo, under `.claude/skills/`):**
- `.claude/skills/shipping-changes/` — `SKILL.md` + `pr-checklist.md` (two tiers; high-risk = judgment-core integrity).
- `.claude/skills/building-and-testing/` — `SKILL.md` (real build/train/eval commands; verified markers).

**Wired together:** `CLAUDE.md` (ceilings + routing) and `docs/STATUS.md` (Done / In-flight / Next,
updated on merge).
...
1. **`CLAUDE.md`** — branch + PR always; never commit to `main`; keep the process lean; plus the real
...
*Provenance: distilled from the `speedrun/` workflow (its `shipping-changes`, `codebase-docs`,
`building-and-testing`, and `working-with-submodules` project skills, the generic superpowers skills,
its always-loaded instruction file, the workflow-tuning design specs, and the overnight-run logs),
then trimmed to this project's §0 answers and re-pointed onto Claude Code (`.claude/skills/`,
`CLAUDE.md`, Skill-tool invocation).*
