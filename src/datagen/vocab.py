...
# NB: every token below is checked disjoint from BLOCKLIST ∪ eval_vocab() by tests/test_vocab.py.
# When adding surfaces, keep them eval-disjoint — the bank exists so the hard-cases set stays a
# clean generalization test. Banks were expanded (v3) so the model learns the person/non-person
# CONTRAST over many surfaces rather than memorizing a handful.
...
COMMON_WORDS: tuple[str, ...] = (
    ...
    "Dale",
    "Wade",
    # v3 additions — each has a genuine COMMON-WORD sense (see author.py COMMON_SENSE), so the
    # person-vs-common minimal pair is a true contrast on the identical surface.
    "Ruby",
    "Hazel",
    "June",
    "Belle",
